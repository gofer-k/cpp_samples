...
struct chunk_data {
  ...
  void work_chunk(barrier_t &sync_point) {
    std::transform(chunk_.begin(), chunk_.end(), chunk_.begin(),
                   [](int i) { return i * 2; });
...
struct producer {
  ...
  void operator()() {
    ...
    for (std::size_t i = 0; i < num_chunks; ++i) {
      product_data chunk;
      std::ranges::copy(data_ | std::views::drop(i * num_chunks) |
                            std::views::take(num_chunks),
    ...
    std::for_each(chunks.begin(), chunks.end(),
                  [](const auto &chunk) { user_utilities:print_vector(chunk.chunk_); });
    const auto total_reduces_values = std::transform_reduce(
        chunks.begin(), chunks.end(), 0, std::plus<int>(),
        [](const auto &chunk) {
...
export void test_barrier_sample() {
  product_data data =
      std::ranges::iota_view{0, 10} | std::ranges::to<product_data>();
